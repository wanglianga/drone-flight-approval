...
- **NO_FLY_ZONE**：航线或轨迹进入通用禁飞区
- **AIRPORT_CLEARANCE**：进入机场净空保护区
- **TEMPORARY_RESTRICTION**：进入临时管制区域
- **SCHOOL_ZONE**：进入学校周边管制区
- **EVENT_ZONE**：进入大型活动管制区
...
每个冲突返回以下详细信息：
- **冲突坐标段**：起止航点序号及经纬度
- **限制高度**：`restrictedMinAltitude` / `restrictedMaxAltitude`
- **可飞时间窗**：`flyableStartTime` / `flyableEndTime`
- **绕行建议**：`detourSuggestion`

### 审批与风险重算

- 审批通过后自动设置报备窗口（计划起飞前30分钟至起飞后15分钟）
- 审批人修改限制后，可调用重新计算风险接口
- 重算风险后若发现新冲突，已审批任务自动回到 `PENDING_REVIEW`（待确认）状态
- 退回修改的任务状态为 `PENDING_REVIEW`

### 起飞报备窗口

- **报备窗口校验**：只能在审批设置的报备窗口内进行起飞报备
- **超时未飞处理**：超过报备窗口未起飞，任务回到待确认状态
- **天气取消**：天气原因取消飞行，任务回到待确认状态，报备记录标记为取消
- **更换无人机**：换机型后任务回到待确认状态，需重新审批
- **更改起降点**：改起降点后任务回到待确认状态，需重新审批

### 重复报备防护

- 同一任务只能存在一条 `REPORTED` 状态的有效报备记录
- 重复提交报备会被标记为 `DUPLICATE` 状态，并抛出业务异常
- 5分钟内相近时间的报备也会被判定为重复
